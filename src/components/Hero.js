...
//desctructure hero code
const Hero = () => {
  const { title, subtitle, btnText, btnIcon } = heroData
  return (
    <section className='bg-hero bg-cover bg-center min-h-[40vh] lg:h-[948px] bg-no-repeat relative mt-[120px] lg:mt-[150px]'>
      <motion.div variants={container} initial="hidden" whileInView={'show'} className='container mx-auto bg-pink-200/30 min-h-[40vh] lg:h-full flex items-center justify-center xl:justify-end'>
        {/* text & btn */}
        ...
        {/* outline text */}
        <div className='hidden xl:flex absolute -bottom-2 right-0 left-0 before:content-outlineTxt'></div>
      </motion.div>
    </section>
  )
};
